...
# sveltos. The kubeconfig is written in the current directory.
#
# You must configure your local kubectl to point to the right k8s cluster and
# have admin-level access.
#
# Note: all of the k8s resources are created in namespace
# "projectsveltos". If you delete any of these objects, sveltos will stop working.
#
set -eu -o pipefail

# Allow passing in common name and username in environment. If not provided,
# use default.
SVELTOS_SA=${SVELTOS_SA_NAME:-projectsveltos-sa}
NAMESPACE=${SVELTOS_NAMESPACE:-projectsveltos}
...
echo "Creating the Kubernetes Service Account with cluster-admin RBAC permissions."
kubectl apply -f - <<EOF
apiVersion: v1
kind: Namespace
metadata:
...
SA_SECRET_NAME=$(kubectl get -n ${NAMESPACE} sa/${SVELTOS_SA} -o "jsonpath={.secrets[0]..name}")
if [ -z $SA_SECRET_NAME ]
then
...
metadata:
  ...
  namespace: ${NAMESPACE}
  annotations:
    kubernetes.io/service-account.name: "${SVELTOS_SA}"
EOF

SA_SECRET_NAME=${SVELTOS_SA}
fi
...
- context:
    ...
    user: ${CURRENT_CLUSTER}-${SVELTOS_SA}
  name: ${CURRENT_CONTEXT}
current-context: ${CURRENT_CONTEXT}
kind: Config
preferences: {}
users:
- name: ${CURRENT_CLUSTER}-${SVELTOS_SA}
  user:
    token: ${SA_TOKEN}
EOF
...
Note: Kubernetes RBAC rules for Projectsveltos were created, you won't need to create them manually."
